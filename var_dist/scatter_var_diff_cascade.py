...
pred_path = '/mnt/truenas/scratch/czh/kl_baseline/experiments/faster_r50_1x_l1_varhead_abs_cascade/coco_val2017_result.json'
coco = COCO(anno_path)
...
def process_img(img_id):
    ...
    for each_pair in match_pair:
        ...
        var = np.array(pred_data['var']).reshape(2, 4)
        bbox = pred_data['bbox']
        ...
        for ii in range(4):
            if ii % 2 == 0:
                ...
                var_data.append(var[:, ii].reshape(1, 2))
            if ii % 2 == 1:
...
plt.scatter(scatter_var.reshape(-1,), scatter_diff.reshape(-1,), s=3, alpha=0.1)
coef = np.corrcoef(scatter_var.reshape(-1,), scatter_diff.reshape(-1,))[0, 1]
plt.grid(True)
plt.xlabel('var')
plt.ylabel('diff')
plt.title('var: ' + str(coef)[:8])

sec_diff = abs(scatter_diff.reshape(-1,) - 0.1 * scatter_var.reshape(-1,))
fig.add_subplot(2, 2, 2)
plt.scatter(sec_var.reshape(-1,), sec_diff.reshape(-1,), s=3, alpha=0.1)
coef = np.corrcoef(sec_var.reshape(-1,), sec_diff.reshape(-1,))[0, 1]
plt.grid(True)
plt.xlabel('sec var')
plt.ylabel('sec diff')
plt.title('sec var: ' + str(coef)[:8])

valid_id = sec_var < 0.5
valid_index = np.where(sec_var < 0.5)
cm = plt.get_cmap('hsv')

fig.add_subplot(2, 2, 3)
plt.scatter(scatter_var.reshape(-1,), scatter_diff.reshape(-1,), s=3, alpha=0.1, c=scatter_var, cmap=cm)
coef = np.corrcoef(scatter_var[valid_index].reshape(-1,), scatter_diff[valid_index].reshape(-1,))[0, 1]
plt.grid(True)
plt.xlabel('var')
plt.ylabel('diff')
plt.title('sec var<0.5: ' + str(coef)[:8])

sec_diff = abs(scatter_diff.reshape(-1,) - 0.1 * scatter_var.reshape(-1,))
fig.add_subplot(2, 2, 4)
plt.scatter(sec_var.reshape(-1,), sec_diff.reshape(-1,), s=3, alpha=0.1, c=scatter_var, cmap=cm)
coef = np.corrcoef(sec_var[valid_index].reshape(-1,), sec_diff[valid_index].reshape(-1,))[0, 1]
plt.grid(True)
plt.xlabel('sec var')
plt.ylabel('sec diff')
plt.title('sec var<0.5: ' + str(coef)[:8])

fig.savefig('img/sec_var_vs_diff.png')
