...
anno_path = "/mnt/truenas/scratch/czh/data/coco/annotations/instances_val2017.json"
pred_path = '/mnt/truenas/scratch/czh/kl_baseline/experiments/faster_r50_1x_l1_varhead_abs/coco_val2017_result.json'
...
def process_img(img_id):
    ...
    for each_pair in match_pair:
        ...
        score_data.append(pred_data['score'])
        for ii in range(4):
            var_data[ii].append(var[ii])
    return np.array(diff_data).reshape(-1, 1), np.array(var_data).transpose(), np.array(score_data).reshape(-1, 1)

import concurrent.futures
diff_list = []; var_list = []; score_list = []
with concurrent.futures.ProcessPoolExecutor(max_workers=50) as executor:
    for index, res_data in enumerate(executor.map(process_img, img_ids)):
        diff_data, var_data, score_data = res_data
...
scatter_score = np.vstack(score_list)

scatter_var = np.mean(scatter_var, axis=1).reshape(-1, 1)

# valid_index = np.where(scatter_var != 0.)[0]
# print(valid_index.shape[0] / scatter_var.shape[0])
...
print(np.corrcoef(scatter_var.reshape(-1,), scatter_diff.reshape(-1,))[0, 1])
plt.scatter(scatter_var, scatter_diff, s=5, alpha=0.2, c=scatter_score, cmap=cm)
# plt.xlim(0, 0.1)
plt.grid(True)
plt.xlabel('var')
plt.ylabel('iou')
plt.colorbar()
fig.savefig('img/l2_woexp_iou_varhead2fc.png', dpi=200)
