...
# pred_path = "/mnt/truenas/scratch/czh/kl_baseline/experiments/faster_r50_1x_l1_varhead_binary/coco_val2017_result_score3.json"
# pred_path = '/mnt/truenas/scratch/czh/xyxy_baseline/experiments/faster_r50v1b_fpn_1x_xyxy/coco_val2017_result.json'
pred_path = '/mnt/truenas/scratch/czh/kl_baseline/experiments/faster_r50_1x_l1_varhead_abs_exp/coco_val2017_result.json'
coco = COCO(anno_path)
...
# calibrate_val = [0.17116, 0.164227, 0.176711, 0.166647]
calibrate_val = [1 for i in range(4)]
def process_img(img_id):
    match_pair = match_pred_gt(img_id)
    diff_data = [[] for ii in range(4)]
    var_data = [[] for ii in range(4)]
    iou_data = []
    for each_pair in match_pair:
        ...
        for ii in range(4):
            if ii % 2 == 0:
                diff_data[ii].append((match_gt_bbox[ii] - bbox[ii] + 1) / width)
                var_data[ii].append(var[ii])
            if ii % 2 == 1:
                diff_data[ii].append((match_gt_bbox[ii] - bbox[ii] + 1) / height)
                var_data[ii].append(var[ii])
    return np.array(diff_data).reshape(4, -1).transpose(), np.array(var_data).reshape(4, -1).transpose(), np.array(iou_data).reshape(-1, 1)
...
scatter_diff = np.mean(scatter_diff, axis=-1)
scatter_var = np.mean(scatter_var, axis=-1)
...
plt.scatter(scatter_var.reshape(-1,) * 0.1, scatter_diff.reshape(-1,), s=3, alpha=0.1)
plt.ylim(0, 0.2)
plt.xlim(0, 0.2)
plt.grid(True)
plt.xlabel('var')
plt.ylabel('diff')
plt.title('var vs diff' + str(coef))
fig.savefig('img/kl_var_vs_diff.png')
